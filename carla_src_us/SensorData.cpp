...
namespace data {
  ...
  std::ostream &operator<<(std::ostream &out, const UltrasonicMeasurement &meas) {
    out << "UltrasonicMeasurement(frame=" << std::to_string(meas.GetFrame())
        << ", timestamp=" << std::to_string(meas.GetTimestamp())
        << ", ranges=" << std::to_string(meas.GetRangeAmount())
        << ')';
    return out;
  }
  ...
  std::ostream &operator<<(std::ostream &out, const UltrasonicRange &range) {
      out << "UltrasonicRange(range=" << std::to_string(range.range)
          << ", angle=" << std::to_string(range.angle)
          << ')';
      return out;
  }
...
void export_sensor_data() {
  ...
  class_<csd::UltrasonicMeasurement, bases<cs::SensorData>, boost::noncopyable, boost::shared_ptr<csd::UltrasonicMeasurement>>("UltrasonicMeasurement", no_init)
    .add_property("raw_data", &GetRawDataAsBuffer<csd::UltrasonicMeasurement>)
    .def("__len__", &csd::UltrasonicMeasurement::size)
    .def("__iter__", iterator<csd::UltrasonicMeasurement>())
    .def("__getitem__", +[](const csd::UltrasonicMeasurement &self, size_t pos) -> csd::UltrasonicRange {
      return self.at(pos);
    })
    .def("__setitem__", +[](csd::UltrasonicMeasurement &self, size_t pos, const csd::UltrasonicRange &detection) {
      self.at(pos) = detection;
    })
    .def(self_ns::str(self_ns::self))
  ;
  
  class_<csd::UltrasonicRange>("UltrasonicRange")
    .def_readwrite("range", &csd::UltrasonicRange::range)
    .def_readwrite("angle", &csd::UltrasonicRange::angle)
    .def(self_ns::str(self_ns::self))
  ;
